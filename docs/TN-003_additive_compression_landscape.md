...
**Author:** CC + Rob
**Date:** 2026-05-07 (Thursday evening preparation; Friday investigation runway)
**Status:** Investigation in progress — Tier 1 documentation tonight, Friday substantive engineering
...
Building the missing `.tern-model` → inference-with-KV-cache → TurboQuant-compressed-cache → perplexity infrastructure is **Friday morning's first deliverable**. Backlog item `load_packed_model rewrite` elevated to HIGH priority commercial-gating accordingly.
...
**Required infrastructure for measurement (Friday)**:
- `load_packed_model` rewrite (elevated to backlog HIGH priority): FP16 branch fix for module-path naming, INT4 branch implementation, `key_mapping` support
- Adapter from `.tern-model` inference path to TurboQuant's `IncrementalTQCompressor` (depends on `past_key_values` shape compatibility post-rewrite)
- Standalone perplexity harness on WikiText-2 (currently PPL is internal to `MixedPrecisionConverter`'s auto-scan, not exposed as measurement API)
...
### Friday morning (3-5 hours): `load_packed_model` rewrite — INFRASTRUCTURE-CRITICAL

Per the elevated backlog item, this gates everything downstream. Substantive work:

1. **FP16 branch fix**: handle module-path naming correctly. Current bug silently skips 853/2130 entries on production manifests because the loader walks `parts[:-1]` then `getattr(parent, parts[-1])` expecting an `nn.Linear` submodule, but production naming gives parameter-path keys (`<module>.weight`, `<module>.bias`).
2. **INT4 branch implementation**: `int4_block32` dtype is currently unhandled in `load_packed_model`'s dispatch (only `ternary2` and `float16` branches exist). 11 entries silently skipped on gemopus-4-e4b; will hit on Phi-4 retrospect compatibility too.
3. **`key_mapping` support**: parameter mirroring `load_as_model`'s existing support, so `GEMMA4_MULTIMODAL_TRANSFORMERS_5_5` and future drift presets work.
4. **Test coverage** against all 5 compressed manifests on disk (Mistral-7B, 26B-A4B Google base, 31B, Qwen3-30B-A3B, Phi-4).
5. **Output sanity**: verification that loaded models produce non-garbage outputs, not just load successfully. Consider quick perplexity sanity check on a tiny held-out sample (~10K tokens) per model as smoke test.
...
1. **TurboQuant baseline against per-expert `.tern-model` artefacts**: 5 models if time permits; degrade gracefully to 3 (Mistral-7B + Qwen3 + Phi-4 as size diversity) if wall-clock pressures.
...
| Phase | Status |
|---|---|
| Tonight (2026-05-07 evening): TN-003 landscape + feasibility scout + Friday plan + backlog elevation | **In flight** |
| Friday morning (2026-05-08): `load_packed_model` rewrite — infrastructure-critical | Planned |
| Friday late morning: Measurement infrastructure (perplexity harness, TurboQuant adapter, smoke test) | Planned |
| Friday afternoon: TurboQuant baseline + KIVI comparison (if feasibility confirmed) | Planned |
| Saturday/following: KVQuant + kvtc + SpQt continuation | Banked |
